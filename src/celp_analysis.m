function [kappa,k,theta0,P,b,ebuf,Zf,Zw] = celp_analysis(x,L,M,c,cb,Pidx,bbuf,ebuf,Zf,Zw)
...
[ar,xi,kappa,ehat] = frame_lpc_analysis(x,M);       % LP analysis of frame.
ac = frame_lpc_weighting(ar,c);                   % Coefficients of filter A(z/c).

for j=1:J                             % Excitation sequence in blocks.
  n = (j-1)*L+1:j*L;
  [k(j),theta0(j),P(j),b(j),ebuf,Zf,Zw] = celp_excitation(x(n),cb,Pidx,ar,ac,...
                                                              bbuf,ebuf,Zf,Zw);
